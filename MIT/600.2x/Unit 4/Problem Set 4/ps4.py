...
def r_squared(y, estimated):
    ...
    estimationErrors = ((y - estimated)**2).sum()
    mean = y.sum()/float(len(y))
    valuesVariability = ((y - mean)**2).sum()
    return 1 - estimationErrors/valuesVariability
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        xVals = pylab.array(x)
        yVals = pylab.array(y)
        pylab.plot(xVals, yVals, 'bo', label='Data points')
        pylab.title(model)
        pylab.xlabel('Year')
        pylab.ylabel('Temperature (ºC)')
        a = list(model)[0]
        b = list(model)[1]
        estYVals = a * xVals + b
        estYVals = pylab.array(estYVals)
        pylab.plot(xVals, estYVals, 'r-', label='Linear fit, R2 = ' + str(round(r_squared(yVals, estYVals), 3)))
